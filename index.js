...
function getAllowedAccountKeys() {
  return new Set([
    normalizeAccountKey(DEFAULT_ACCOUNT_KEY),
...
function getAllowedAdminIds() {
  ...
  [
    ...
    process.env.SECOND_ACCOUNT_BUSINESS_OWNER_ID
  ].forEach(id => addId(ids, id));
  if (typeof ENV_ACCOUNTS !== 'undefined') {
    for (const account of ENV_ACCOUNTS || []) {
      addId(ids, account.admin_chat_id);
      addId(ids, account.owner_user_id);
      addId(ids, account.business_owner_id);
    }
  }
  return ids;
}

function isAllowedAdminId(telegramUserId) {
  return getAllowedAdminIds().has(String(telegramUserId || '').trim());
}

function isAllowedAccountKey(accountOrKey) {
  return getAllowedAccountKeys().has(normalizeAccountKey(accountKey(accountOrKey)));
}

function canManageAccount({ telegramUserId, accountKey: rawAccountKey }) {
  const allowedAdminIds = getAllowedAdminIds();
  const allowedAccountKeys = getAllowedAccountKeys();
  return (
    allowedAdminIds.has(String(telegramUserId || '').trim()) &&
    allowedAccountKeys.has(normalizeAccountKey(rawAccountKey))
  );
}

function accountKeyMatches(a, b) {
  return normalizeAccountKey(a) === normalizeAccountKey(b);
}

function accountSetHas(set, key) {
  return [...(set || [])].some(value => accountKeyMatches(value, key));
}
...
function accountDisplayLabel(accountOrKey = DEFAULT_ACCOUNT_KEY) {
  ...
  const normalized = normalizeAccountKey(ak);
  if (normalized === normalizeAccountKey(process.env.SECOND_ACCOUNT_KEY || 'second')) return process.env.SECOND_ACCOUNT_LABEL || 'Liderlar';
  if (normalized === DEFAULT_ACCOUNT_KEY) return 'UZLYE';
  return ak;
}
...
function commandManagementTargetAccountKey(accountOrKey = DEFAULT_ACCOUNT_KEY) {
  return normalizeAccountKey(accountKey(accountOrKey));
}

function isCommandManagementAccountKey(accountOrKey = DEFAULT_ACCOUNT_KEY) {
  return isAllowedAccountKey(accountOrKey) || COMMAND_MANAGEMENT_ACCOUNT_KEYS.has(commandManagementTargetAccountKey(accountOrKey));
}
...
async function sendBusinessMessage(lead, text, { traceId = '', templateKey = '' } = {}) {
  ...
  console.log(`[TELEGRAM_SEND_ATTEMPT] trace_id=${traceId || '-'} sender_type=bot account_key=${lead.account_key} chat_id=${lead.chat_id} business_connection_id=${lead.business_connection_id} template_key=${templateKey || '-'}`);
  let result;
  try {
    ...
    console.log(`[TELEGRAM_SEND_RESULT] trace_id=${traceId || '-'} sender_type=bot ok=false telegram_message_id=- error_code=${err?.telegram?.error_code || '-'} error_description=${String(err?.telegram?.description || err?.message || '-').replace(/\s+/g, '_').slice(0, 240)}`);
    throw err;
  }
  if (!result?.message_id) {
    const err = new Error('Telegram sendMessage returned no message_id');
    console.log(`[TELEGRAM_SEND_RESULT] trace_id=${traceId || '-'} sender_type=bot ok=false telegram_message_id=- error_code=missing_message_id error_description=telegram_result_missing_message_id`);
    throw err;
  }
  console.log(`[TELEGRAM_SEND_RESULT] trace_id=${traceId || '-'} sender_type=bot ok=true telegram_message_id=${result?.message_id || '-'}`);
...
function isTestAdminAllowed(chatId, fromId = '') {
  ...
  return idSetHas(TEST_ADMIN_IDS, chatId, fromId) || isAllowedAdminId(chatId) || isAllowedAdminId(fromId);
}
...
async function getTemplate(key, accountOrKey = DEFAULT_ACCOUNT_KEY, { traceId = '', fallbackUsed = false, fallbackReason = '-' } = {}) {
  ...
  if (!tenant?.workspace_id || !tenant?.account_id) {
    console.log(`[TENANT_SCOPE_REJECTED] source=template_repository account_key=${ak} template_key=${key} reason=tenant_scope_unresolved`);
    return null;
  }
...
function isCommandArchived(command = {}) {
  const description = String(command.description || '').trim().toLowerCase();
  const title = String(command.title || '').trim().toLowerCase();
  return command.is_archived === true ||
    description.startsWith('[archived]') ||
...
async function getCustomCommands(accountOrKey = DEFAULT_ACCOUNT_KEY, includeDisabled = false) {
  ...
  const commands = data || [];
  return includeDisabled ? commands : commands.filter(command => !isCommandArchived(command));
}
...
async function getCustomCommand(accountOrKey, commandKey, includeDisabled = true) {
  ...
  if (!includeDisabled && data && isCommandArchived(data)) return null;
  return data || null;
}
...
async function executeCustomCommand(command, lead, text) {
  if (!command || command.is_enabled === false || isCommandArchived(command)) return false;
...
async function customCommandsDebugText(accountOrKey, session = null) {
  ...
  const enabled = commands.filter(c => c.is_enabled !== false && !isCommandArchived(c));
  ...
  return (
    ...
    (latest.length
      ? latest.map(c => `/${c.command_key} | ${c.trigger_type || '-'} | ${c.response_type || '-'} | enabled:${c.is_enabled !== false}`).join('\n')
      : 'Command yo‘q.')
  );
}

function stripArchiveMarker(value = '') {
  const clean = String(value || '').replace(/^\[archived\]\s*/i, '').trim();
  return clean === 'soft_deleted' ? '' : clean;
}

function addArchiveMarker(value = '') {
  const clean = stripArchiveMarker(value);
  return clean ? `[archived] ${clean}` : '[archived]';
}

async function commandResponseDetail(command = {}, accountOrKey = DEFAULT_ACCOUNT_KEY) {
  const responseType = command.response_type || 'text';
  if (responseType === 'template' && command.template_key) {
    const body = await getTemplate(command.template_key, accountOrKey);
    return `Shablon key: ${command.template_key}\n\n${body || '-'}`;
  }
  if (responseType === 'template_sequence') {
    const keys = safeJsonArray(command.template_sequence).length
      ? safeJsonArray(command.template_sequence)
      : String(command.response_text || '').split(/[,\s]+/).filter(Boolean);
    if (!keys.length) return 'Shablonlar: -';
    const bodies = [];
    for (const key of keys.slice(0, 5)) {
      const body = await getTemplate(key, accountOrKey);
      bodies.push(`${key}:\n${body || '-'}`);
    }
    return `Shablonlar: ${keys.join(', ')}\n\n${bodies.join('\n\n')}`;
  }
  if (responseType === 'flow_step') return `Flow step: ${command.step_key || command.response_text || '-'}`;
  if (responseType === 'human_needed') return 'Admin javob bersin.';
  if (responseType === 'silent') return 'Jim turish.';
  return command.response_text || '-';
}

async function updateCustomCommandMetadata(accountOrKey, commandKey, patch = {}) {
  const sanitized = sanitizeCommandKey(commandKey);
  if (!sanitized.ok) throw new Error(sanitized.error);
  const { data, error } = await supabase.from('account_custom_commands')
    .update(patch)
...
function isCommandArchiveSchemaError(error) {
  const msg = String(error?.message || '');
  return error?.code === 'PGRST204' ||
    error?.code === 'PGRST205' ||
    ['is_archived', 'archived_at', 'archived_by'].some(field => msg.includes(field));
}

async function setCommandArchiveState(accountOrKey, commandKey, archived, actorId = '') {
  const account = await getAccount(accountOrKey);
  ...
  const patch = archived
    ? { is_archived: true, archived_at: now, archived_by: actor, is_enabled: false, updated_by: actor, updated_at: now }
    : { is_archived: false, archived_at: null, archived_by: null, is_enabled: true, updated_by: actor, updated_at: now };
  try {
    const updated = await updateCustomCommandMetadata(account.account_key, command.command_key, patch);
    return updated || { ...command, ...patch };
  } catch (err) {
    if (!isCommandArchiveSchemaError(err)) throw err;
    const fallback = archived
      ? {
          ...command,
          is_enabled: false,
          title: addArchiveMarker(command.title || `/${command.command_key}`),
          description: addArchiveMarker(command.description || '')
        }
      : {
          ...command,
          is_enabled: true,
          title: stripArchiveMarker(command.title || '') || `/${command.command_key}`,
          description: stripArchiveMarker(command.description || '') || null
        };
    return upsertCustomCommand(account.account_key, fallback, actorId);
  }
}

async function saveCommandTemplateEdit(accountOrKey, commandKey, body, actorId = '') {
  const account = await getAccount(accountOrKey);
  const command = await getCustomCommand(account.account_key, commandKey, true);
  if (!command) return null;
  const now = new Date().toISOString();
  const actor = actorId ? String(actorId) : null;
  if (command.response_type === 'template' && command.template_key) {
    await setTemplate(command.template_key, String(body || ''), account.account_key);
    const updated = await updateCustomCommandMetadata(account.account_key, command.command_key, {
      updated_by: actor,
      updated_at: now
    });
    return updated || { ...command, updated_by: actor, updated_at: now };
  }
  return upsertCustomCommand(account.account_key, {
    ...command,
    response_type: 'text',
    response_text: String(body || '').slice(0, 3500),
    template_key: null,
    template_sequence: []
  }, actorId);
}

function commandTriggerTypeFromCallback(data = '') {
  const aliases = {
    command_trigger_slash: 'slash_command',
    command_trigger_keyword: 'keyword',
    command_trigger_exact: 'exact_text',
    command_trigger_contains: 'contains_text',
    command_trigger_ai: 'ai_intent'
  };
  if (data.startsWith('cmd_trigger:')) return data.split(':')[1];
  return aliases[data] || null;
}

function commandResponseTypeFromCallback(data = '') {
  const aliases = {
    command_response_text: 'text',
    command_response_template: 'template',
    command_response_template_sequence: 'template_sequence',
    command_response_flow_step: 'flow_step',
    command_response_human_needed: 'human_needed',
    command_response_silent: 'silent'
  };
  if (data.startsWith('cmd_response:')) return data.split(':')[1];
  return aliases[data] || null;
}

function commandTriggerKeyboard() {
  return {
    inline_keyboard: [
      [{ text: '🔹 Slash command', callback_data: 'command_trigger_slash' }],
      [{ text: '🔹 Kalit so‘z', callback_data: 'command_trigger_keyword' }],
      [{ text: '🔹 Aniq matn', callback_data: 'command_trigger_exact' }],
      [{ text: '🔹 Matn ichida bo‘lsa', callback_data: 'command_trigger_contains' }],
      [{ text: '🔹 AI intent', callback_data: 'command_trigger_ai' }],
      [{ text: '❌ Bekor qilish', callback_data: 'command_cancel' }]
    ]
  };
}

function commandResponseKeyboard() {
  return {
    inline_keyboard: [
      [{ text: '📝 Oddiy matn', callback_data: 'command_response_text' }],
      [{ text: '📄 Shablon', callback_data: 'command_response_template' }],
      [{ text: '📚 Bir nechta shablon', callback_data: 'command_response_template_sequence' }],
      [{ text: '🔁 Flow bosqichi', callback_data: 'command_response_flow_step' }],
      [{ text: '👤 Admin javob bersin', callback_data: 'command_response_human_needed' }],
      [{ text: '🤫 Jim turish', callback_data: 'command_response_silent' }],
      [{ text: '❌ Bekor qilish', callback_data: 'command_cancel' }]
    ]
  };
}

async function saveCommandCreateSession(chatId, accountKeyForSetup, step, payload = {}) {
  const nextPayload = {
    ...payload,
    selected_account_key: accountKeyForSetup,
...
async function startCommandCreate(chatId, accountOrKey, from = {}) {
  const account = await getAccount(accountOrKey);
  if (!account?.account_key || account.account_key === UNKNOWN_ACCOUNT_KEY) {
    return tg('sendMessage', { chat_id: chatId, text: 'Avval akkauntni tanlang.' });
  }
  ...
  return tg('sendMessage', { chat_id: chatId, text: 'Buyruq nomini yuboring. Masalan: narx, manzil, aloqa' });
}

async function showCommandCreatePreview(chatId, payload = {}) {
  const account = await getAccount(payload.selected_account_key || payload.account_key);
  const command = {
  ...
  try {
    ...
    console.error('command preview state save failed:', err.message);
    return tg('sendMessage', { chat_id: chatId, text: `Xatolik: buyruq yaratish jarayoni davom etmadi. Sabab: ${err.message}` });
  }
  return tg('sendMessage', {
    chat_id: chatId,
...
async function handleCommandCreateInput({ chatId, from = {}, text, session, selectedAccountKey, ownedAccountKeys, denyAccount }) {
  ...
  if (!accountSetHas(ownedAccountKeys, targetAccountKey)) return denyAccount();
  ...
  try {
    ...
    if (step === 'flow_step') {
      return showCommandCreatePreview(chatId, { ...payload, step_key: sanitizeSetupKey(text, 64) });
    }
    return tg('sendMessage', { chat_id: chatId, text: 'Xatolik: buyruq yaratish jarayoni davom etmadi. Sabab: noto‘g‘ri bosqich.' });
  } catch (err) {
    console.error('handleCommandCreateInput:', err.message);
    return tg('sendMessage', { chat_id: chatId, text: `Xatolik: buyruq yaratish jarayoni davom etmadi. Sabab: ${err.message}` });
  }
}


async function getFlowSteps(accountOrKey = DEFAULT_ACCOUNT_KEY, { traceId = '', stepKeys = [] } = {}) {
  const account = await getAccount(accountOrKey);
  const tenant = await resolveTenantContext({
    accountKey: account.account_key,
    businessConnectionId: account.business_connection_id
  ...
  if (!tenant?.workspace_id || !tenant?.account_id) {
    console.log(
      `[TENANT_SCOPE_REJECTED] trace_id=${traceId || '-'} source=flow_repository ` +
      `account_key=${account.account_key} workspace_id=- workspace_business_account_id=- ` +
      `step_key=${stepKeys.join(',') || '-'} reason=tenant_scope_unresolved`
    );
    return [];
  }
  const resolvedFlowKeys = resolveInfoFlowKeys(account);
  let q = supabase.from('account_flow_steps')
    .select('*')
    .eq('account_key', account.account_key)
    .in('flow_key', resolvedFlowKeys)
    .eq('workspace_id', tenant.workspace_id)
    .eq('workspace_business_account_id', tenant.account_id)
    .eq('is_active', true)
    .order('sort_order', { ascending: true });
  if (stepKeys.length) q = q.in('step_key', stepKeys);
  const { data, error } = await q;
  console.log(
    `[FLOW_QUERY] trace_id=${traceId || '-'} table=account_flow_steps ` +
    `account_key=${account.account_key} resolved_flow_keys=${resolvedFlowKeys.join(',') || '-'} requested_step_key=${stepKeys[0] || '-'} step_candidates=${stepKeys.join(',') || '-'} ` +
    `workspace_id=${tenant.workspace_id} workspace_business_account_id=${tenant.account_id} row_count=${data?.length || 0} ` +
    `error=${error?.code || '-'}:${error ? String(error.message).replace(/\s+/g, '_').slice(0, 160) : '-'}`
  );
  if (error) {
...
function resolveInfoFlowKeys(account = {}) {
  const values = [];
  const rawFlowKey = String(account.flow_key || '').trim();
  const resolvedAccountKey = normalizeAccountKey(accountKey(account));
  if (rawFlowKey) values.push(rawFlowKey.endsWith('_info_only') ? rawFlowKey : `${rawFlowKey}_info_only`);
  if (resolvedAccountKey) values.push(`${resolvedAccountKey}_info_only`);
  return [...new Set(values.filter(Boolean))];
}

async function getFlowStep(accountOrKey, stepKey, traceId = '') {
  const steps = await getFlowSteps(accountOrKey, { traceId, stepKeys: [stepKey] });
  return steps.find(s => s.step_key === stepKey) || null;
}

const CANONICAL_FLOW_STEP_ALIASES = {
  ask_info: ['ask_info', 'asked_info', 'info', 'info_question']
};

async function flowTemplateKeys(accountOrKey, stepKey, fallbackKeys = [], traceId = '') {
  const candidates = [...new Set([stepKey, ...(CANONICAL_FLOW_STEP_ALIASES[stepKey] || [])])];
  const steps = await getFlowSteps(accountOrKey, { traceId, stepKeys: candidates });
  const step = candidates.map(candidate => steps.find(row => row.step_key === candidate)).find(Boolean);
  if (step?.template_key) {
    const keys = String(step.template_key).split(/[,\s]+/).map(x => x.trim()).filter(Boolean);
    const usable = [];
    for (const key of keys) if (await getTemplate(key, accountOrKey, { traceId, fallbackUsed: false })) usable.push(key);
    console.log(`[TEMPLATE_RESOLUTION] trace_id=${traceId || '-'} template_key=${keys.join(',') || '-'} tenant_template_found=${usable.length ? 'true' : 'false'} fallback_used=false fallback_reason=${usable.length ? '-' : 'template_missing'}`);
    return usable;
  }
  const canonicalTemplateKeys = [...new Set([...fallbackKeys, ...candidates])];
  const available = [];
  for (const key of canonicalTemplateKeys) {
    if (await getTemplate(key, accountOrKey, { traceId, fallbackUsed: true, fallbackReason: 'flow_step_missing' })) available.push(key);
  }
  if (available.length) {
    console.log(`[TEMPLATE_RESOLUTION] trace_id=${traceId || '-'} template_key=${available.join(',')} tenant_template_found=true fallback_used=true fallback_reason=flow_step_missing`);
    return available;
  }
  console.log(`[TEMPLATE_RESOLUTION] trace_id=${traceId || '-'} template_key=${stepKey} tenant_template_found=false fallback_used=true fallback_reason=flow_or_template_missing`);
  console.log(`[TENANT_SCOPE_REJECTED] trace_id=${traceId || '-'} source=flow_repository account_key=${accountKey(accountOrKey) || '-'} step_key=${stepKey} reason=flow_step_missing`);
  return [];
}
...
async function getAccountAiRules(accountOrKey = DEFAULT_ACCOUNT_KEY) {
  const account = await getAccount(accountOrKey);
  if (account.ai_rules_enabled === false) return [];
...
async function sendTemplate(lead, templateKey, { traceId = '' } = {}) {
  const body = await getTemplate(templateKey, lead.account_key, { traceId });
  if (!body) {
  ...
  const sent = await sendBusinessMessage(lead, text, { traceId, templateKey });
...
async function sendPackage(lead, actionName, templateKeys, nextPatch = {}, { traceId = '' } = {}) {
  const allowed = await canLeadAutoReply(lead, lead.account_key);
  if (!allowed.ok) {
  ...
  if (!templateKeys.length) {
    await logEvent(lead.chat_id, 'flow_step_missing', actionName, lead.account_key);
    console.log(`[FLOW_CONTINUE] trace_id=${traceId || '-'} sender_type=bot current_stage=${lead.stage || '-'} next_stage=${nextPatch.stage || '-'} template_found=false fallback_used=false send_ok=false telegram_message_id=- stop_reason=empty_package`);
    return null;
  }
  ...
  for (const key of templateKeys) {
    const sent = await sendTemplate(currentLead, key, { traceId });
    if (!sent) {
      console.log(`[FLOW_CONTINUE] trace_id=${traceId || '-'} sender_type=bot current_stage=${lead.stage || '-'} next_stage=${nextPatch.stage || '-'} template_found=true send_ok=false stop_reason=telegram_send_failed`);
      return null;
    }
    telegramMessageId = sent.message_id || telegramMessageId;
    await sleep(350);
  }
...
async function markOutreach({ chatId, businessConnectionId, from, text, messageId = null, accountKey = DEFAULT_ACCOUNT_KEY }) {
  ...
  if (!messageId) {
    await logEvent(chatId, 'outreach_tracking_failed', 'admin_outgoing_message_id_missing', accountKey);
    return false;
  }
  ...
  console.log(
    ...
    `lead_chat_id=${chatId} ` +
    `sender_type=admin telegram_message_observed=true ` +
...
async function handleBusinessMessage(msg, options = {}) {
  ...
  if (isAccountOwnerMessage(msg, account)) {
    ...
    console.log(
      `[BUSINESS_OUTGOING] trace_id=${traceId} sender_type=admin account_key=${ak} lead_chat_id=${chatId} message_id=${msg.message_id || '-'} ` +
...
async function processLeadBatch(initialLead, texts, traceId = '') {
  ...
  console.log(
    `[INTENT_DECISION] trace_id=${traceId || '-'} account_key=${lead.account_key} ` +
  ...
  if (lead.stage === STAGE.OUTREACH_SENT || lead.stage === STAGE.NEW || lead.stage === STAGE.PENDING_APPROVAL) {
    const keys = await flowTemplateKeys(lead.account_key, 'ask_application', ['ask_application'], traceId);
    console.log(`[FLOW_CONTINUE] trace_id=${traceId || '-'} sender_type=bot current_stage=${lead.stage || '-'} next_stage=${STAGE.ASKED_APPLICATION} template_found=${keys.length ? 'true' : 'false'} send_ok=pending`);
    const after = await sendPackage(lead, 'ask_application', keys, { stage: STAGE.ASKED_APPLICATION, status: 'active', bot_enabled: true, bot_enabled_for_lead: true }, { traceId });
    console.log(`[FLOW_CONTINUE] trace_id=${traceId || '-'} sender_type=bot current_stage=${lead.stage || '-'} next_stage=${STAGE.ASKED_APPLICATION} template_found=${keys.length ? 'true' : 'false'} send_ok=${after ? 'true' : 'false'} stop_reason=${after ? '-' : 'flow_step_missing_or_send_failed'}`);
    return;
  }
...
async function sendAccountDebug(chatId, telegramUserId, selectedAccountKey = DEFAULT_ACCOUNT_KEY) {
  ...
  const normalized = normalizeAccountKey(sessionSelected);
  const owned = await getPublicOwnedAccounts(chatId, telegramUserId);
  const effectiveCanManage = accountSetHas(new Set(owned.map(a => a.account_key)), sessionSelected);
  const allowedAdmin = isAllowedAdminId(telegramUserId) || Boolean(owned.length);
  const allowedAccount = isAllowedAccountKey(sessionSelected);
  const canManage = canManageAccount({ telegramUserId, accountKey: sessionSelected }) || effectiveCanManage;
  return tg('sendMessage', {
    chat_id: chatId,
    text:
      `account_debug\n\n` +
      ...
      `selected_account_key: ${sessionSelected || '-'}\n` +
      `normalized_account_key: ${normalized || '-'}\n` +
      `SECOND_ACCOUNT_KEY: ${process.env.SECOND_ACCOUNT_KEY || 'second'}\n` +
...
async function sendReachStartPreview(chatId, telegramUserId, selectedAccountKey = DEFAULT_ACCOUNT_KEY) {
  const allowedAccountKey = await getCommandManagementAccountKey(chatId, telegramUserId, selectedAccountKey);
  if (!allowedAccountKey) {
    return isAllowedAdminId(telegramUserId)
      ? tg('sendMessage', { chat_id: chatId, text: ACCOUNT_NOT_SELECTED_TEXT })
  ...
  if (!canAccountAutoReply(account)) {
    return tg('sendMessage', {
      ...
      text:
        ...
        `Bot o‘chirilgan. Avval yoqing.`,
      reply_markup: { inline_keyboard: [[{ text: '▶️ Botni yoqish', callback_data: 'account_bot:on' }], [{ text: '❌ Bekor qilish', callback_data: 'reach_start_cancel' }]] }
    });
  }
  ...
  return tg('sendMessage', {
    ...
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: '❌ Bekor qilish', callback_data: 'reach_start_cancel' }]
      ]
    }
  });
}

async function executeReachStart(chatId, telegramUserId) {
  const session = await getAdminSession(chatId);
  const payload = session?.payload || {};
...
async function parseOptionalAccountArg(parts, selectedAccountKey, startIndex = 1, allowedAccountKeys = null) {
  ...
  const maybeNormalized = normalizeAccountKey(maybe);
  const account = accounts.find(a => accountKeyMatches(a.account_key, maybeNormalized));
  const allowed = allowedAccountKeys
    ? [...allowedAccountKeys].some(key => accountKeyMatches(key, account?.account_key || maybeNormalized))
    : true;
  if ((account || isAllowedAccountKey(maybe)) && !allowed) {
    return { accountKey: null, nextIndex: startIndex + 1, denied: true };
  }
  const recognizedAccount = account || (isAllowedAccountKey(maybe) ? { account_key: maybeNormalized } : null);
  return {
    accountKey: recognizedAccount?.account_key || selectedAccountKey,
    nextIndex: recognizedAccount ? startIndex + 1 : startIndex
  };
}
...
async function getSelectedAccountKey(adminChatId) {
  const { data, error } = await supabase.from('admin_sessions').select('account_key,payload').eq('chat_id', String(adminChatId)).maybeSingle();
  if (!error && (data?.payload?.selected_account_key || data?.account_key)) return data.payload?.selected_account_key || data.account_key;
  const accounts = await getAccounts();
...
async function getPublicOwnedAccounts(chatId, fromId = '') {
  ...
  const addAllowedAccount = (account) => {
    if (!account?.account_key) return;
    if (!allowedAccountKeys.has(normalizeAccountKey(account.account_key))) return;
    byKey.set(account.account_key, account);
  };

  if (isAllowedAdminId(from) || isAllowedAdminId(chat)) {
    for (const account of accounts) addAllowedAccount(account);
    for (const key of allowedAccountKeys) {
      if (![...byKey.values()].some(a => accountKeyMatches(a.account_key, key))) {
        addAllowedAccount(await getAccount(key));
      }
    }
  }
  ...
  if (ids.length) {
    ...
    if (!error) {
      for (const row of data || []) {
        const account = accounts.find(a => accountKeyMatches(a.account_key, row.account_key)) || await getAccount(row.account_key);
        addAllowedAccount(account);
      }
    } else if (!String(error.message || '').includes('does not exist')) {
...
async function requirePublicAccountAccess(chatId, fromId, requestedAccountKey) {
  const owned = await getPublicOwnedAccounts(chatId, fromId);
  const requested = normalizeAccountKey(requestedAccountKey);
  const found = owned.find(a => accountKeyMatches(a.account_key, requested));
  if (found?.account_key) return found.account_key;
  if (canManageAccount({ telegramUserId: fromId, accountKey: requested })) return requested;
  return null;
}

async function getCommandManagementAccountKey(chatId, fromId, preferredAccountKey = DEFAULT_ACCOUNT_KEY) {
  const targetAccountKey = commandManagementTargetAccountKey(preferredAccountKey);
  if (!isCommandManagementAccountKey(targetAccountKey)) return null;
  const allowedKey = await requirePublicAccountAccess(chatId, fromId, targetAccountKey);
  if (allowedKey) await setSelectedAccountKey(chatId, allowedKey, fromId);
  return allowedKey;
}

async function canUseCommandManagement(chatId, fromId, preferredAccountKey = DEFAULT_ACCOUNT_KEY) {
  return Boolean(await getCommandManagementAccountKey(chatId, fromId, preferredAccountKey));
}

function commandManagementDenied(chatId) {
  return tg('sendMessage', { chat_id: chatId, text: COMMAND_MANAGEMENT_DENIED_TEXT });
}

function isCommandManagementCallbackData(data = '') {
  return [
    'cmd_menu',
    'commands_menu',
    'cmd_list',
    ...
    'command_cancel',
    'cmd_cancel'
  ].includes(data) || data.startsWith('cmd_') || data.startsWith('command_');
}

async function upsertAdminSessionRow(row) {
  let { error } = await supabase.from('admin_sessions').upsert(row);
  if (error && String(error.message || '').includes('telegram_user_id')) {
    const fallback = { ...row };
    delete fallback.telegram_user_id;
    ({ error } = await supabase.from('admin_sessions').upsert(fallback));
  }
  return error;
}

async function setSelectedAccountKey(adminChatId, selectedAccountKey, telegramUserId = '') {
  const normalizedKey = normalizeAccountKey(selectedAccountKey);
  const row = {
    chat_id: String(adminChatId),
    telegram_user_id: String(telegramUserId || adminChatId),
    mode: 'account_selected',
    account_key: normalizedKey,
    payload: {
      telegram_user_id: String(telegramUserId || adminChatId),
      chat_id: String(adminChatId),
      selected_account_key: normalizedKey,
      account_key: normalizedKey
    },
    updated_at: new Date().toISOString()
  };
  const error = await upsertAdminSessionRow(row);
...
async function setAdminSession(chatId, mode, payload = {}) {
  const selectedAccountKey = payload.selected_account_key || payload.account_key || null;
  const row = {
    chat_id: String(chatId),
    telegram_user_id: payload.telegram_user_id ? String(payload.telegram_user_id) : undefined,
    mode,
    account_key: selectedAccountKey ? normalizeAccountKey(selectedAccountKey) : null,
    payload,
    updated_at: new Date().toISOString()
  };
  if (!row.telegram_user_id) delete row.telegram_user_id;
  const error = await upsertAdminSessionRow(row);
  if (error) {
...
async function sendAccountsMenu(chatId, fromId = chatId) {
  const accounts = await getPublicOwnedAccounts(chatId, fromId);
  const selected = await getPublicSelectedAccountKey(chatId, fromId);
...
async function sendCommandsMenu(chatId, accountOrKey = DEFAULT_ACCOUNT_KEY, options = {}) {
  const account = await getAccount(accountOrKey);
  const archived = Boolean(options.archived);
  const allCommands = await getCustomCommands(account.account_key, true);
  ...
  for (const cmd of commands.slice(0, 20)) {
    rows.push([{ text: `${archived ? '📦' : '🟢'} /${cmd.command_key}`, callback_data: `cmd_view:${cmd.command_key}` }]);
  }
  rows.push([{ text: '⬅️ Orqaga', callback_data: 'menu' }]);
  const title = archived ? '📦 Arxivdagi buyruqlar' : '🧩 Buyruqlar';
  const empty = archived ? 'Arxivda buyruq yo‘q.' : 'Faol buyruq yo‘q.';
  return tg('sendMessage', {
    chat_id: chatId,
    text: `Joriy akkaunt: ${account.label || account.account_key}\n\n${title}\n\n${commands.length ? 'Buyruqni tanlang.' : empty}`,
    reply_markup: { inline_keyboard: rows }
  });
}
...
async function sendCommandDetail(chatId, accountOrKey, commandKey) {
  ...
  const archived = isCommandArchived(command);
  const response = await commandResponseDetail(command, account.account_key);
  const rows = [
    [{ text: '✏️ Shablonni tahrirlash', callback_data: `cmd_edit_template:${command.command_key}` }],
    archived
      ? [{ text: '♻️ Qayta tiklash', callback_data: `cmd_restore:${command.command_key}` }]
      : [{ text: '📦 Arxivlash', callback_data: `cmd_archive:${command.command_key}` }],
    [{ text: '⬅️ Orqaga', callback_data: archived ? 'cmd_archive_list' : 'cmd_list' }]
  ];
  return tg('sendMessage', {
    chat_id: chatId,
    text:
      `Buyruq: /${command.command_key}\n` +
      `Holati: ${archived ? 'arxiv' : 'faol'}\n` +
      `Admin profil: ${account.label || account.account_key} (${account.account_key})\n\n` +
      `Hozirgi javob shabloni:\n${short(response, 3000)}`,
    reply_markup: {
...
async function sendAiRulesMenu(chatId, accountOrKey = DEFAULT_ACCOUNT_KEY) {
  ...
  const rows = [
    ...
    [{ text: '🤖 AI orqali qoida yaratish', callback_data: 'airule_ai_create' }],
...
async function sendDashboard(chatId, accountOrKey = DEFAULT_ACCOUNT_KEY, fromId = chatId) {
  const account = await getAccount(accountOrKey);
  const accounts = await getPublicOwnedAccounts(chatId, fromId);
  const auto = await getAutoOutreach(account.account_key);
  const daily = await getDailyAuto(account.account_key);
  const showCommands = await canUseCommandManagement(chatId, fromId, account.account_key);
...
async function sendPlatformCommandTest(chatId, accountOrKey, sampleText) {
  return adminTg('sendMessage', {
    ...
    text:
      `🧭 Platform Admin Bot\n\n` +
      (await customCommandDryRunText(accountOrKey, sampleText))
  });
}
...
async function handlePlatformAdminMessage(msg) {
  ...
  if (text.startsWith('/commands_debug ')) {
    const [, ak] = text.split(/\s+/);
    return adminTg('sendMessage', { chat_id: chatId, text: `🧭 Platform Admin Bot\n\n${await customCommandsDebugText(ak)}` });
  }
  ...
  if (text.startsWith('/settemplate ')) {
    ...
    const [ak, key] = rest.split(/\s+/, 2);
    const body = rest.slice(`${ak} ${key}`.length).trim();
    if (!ak || !key || !body) return adminTg('sendMessage', { chat_id: chatId, text: 'Format: /settemplate ACCOUNT_KEY TEMPLATE_KEY TEXT' });
    const before = { body: await getTemplate(key, ak) };
    await setTemplate(key, body, ak);
    await logPlatformAudit({ adminUserId: from.id, adminUsername: from.username, action: 'template_set', targetAccountKey: ak, beforeJson: before, afterJson: { key, body } });
    return adminTg('sendMessage', { chat_id: chatId, text: `🧭 Platform Admin Bot\n\n✅ Template saqlandi: ${ak}/${key}` });
  }
  if (text.startsWith('/setflow ')) {
    const [, ak, stepKey, templateKey, nextYes, nextNo, nextPartial, nextUnknown, stopAfterSend] = text.split(/\s+/);
    if (!ak || !stepKey || !templateKey) return adminTg('sendMessage', { chat_id: chatId, text: 'Format: /setflow ACCOUNT_KEY STEP_KEY TEMPLATE_KEY NEXT_YES NEXT_NO NEXT_PARTIAL NEXT_UNKNOWN STOP_TRUE_FALSE' });
    const before = await getFlowStep(ak, stepKey);
    await upsertFlowStep({ accountKey: ak, stepKey, templateKey, nextYes, nextNo, nextPartial, nextUnknown, stopAfterSend });
    const after = await getFlowStep(ak, stepKey);
    await logPlatformAudit({ adminUserId: from.id, adminUsername: from.username, action: 'flow_step_set', targetAccountKey: ak, beforeJson: before || {}, afterJson: after || { stepKey, templateKey } });
    return adminTg('sendMessage', { chat_id: chatId, text: `🧭 Platform Admin Bot\n\n✅ Flow step saqlandi: ${ak}/${stepKey}` });
  }
  if (text.startsWith('/setairule ')) {
    const [, ak, ruleKey, stepKey, intent, action] = text.split(/\s+/);
    if (!ak || !ruleKey || !stepKey || !intent || !action) return adminTg('sendMessage', { chat_id: chatId, text: 'Format: /setairule ACCOUNT_KEY RULE_KEY STEP_KEY INTENT ACTION' });
    const before = (await getAccountAiRules(ak)).find(r => r.rule_key === ruleKey) || {};
    const saved = await upsertAccountAiRule(ak, { rule_key: ruleKey, step_key: stepKey, target_intent: intent, action }, from.id);
    await logPlatformAudit({ adminUserId: from.id, adminUsername: from.username, action: 'ai_rule_set', targetAccountKey: ak, beforeJson: before, afterJson: saved });
    return adminTg('sendMessage', { chat_id: chatId, text: `🧭 Platform Admin Bot\n\n✅ AI rule saqlandi: ${saved.account_key}/${saved.rule_key}` });
  }
...
async function handleAdminMessage(msg) {
  ...
  const publicAccountArg = async (maybeAccountKey, fallback = selectedAccountKey) => {
    ...
    const accounts = await getAccounts();
    const account = accounts.find(a => accountKeyMatches(a.account_key, maybeAccountKey));
    if (!account) return fallback;
    return [...ownedAccountKeys].some(key => accountKeyMatches(key, account.account_key)) ? account.account_key : null;
  };
  ...
  if (text === '/cancel') {
    await setSelectedAccountKey(chatId, selectedAccountKey, from.id);
    return sendDashboard(chatId, selectedAccountKey, from.id);
  }

  if ((session?.mode === 'editing_command_template' || session?.mode === 'custom_command_edit_response') && !text.startsWith('/')) {
    const payload = session.payload || {};
    const targetAccountKey = payload.selected_account_key || payload.account_key || selectedAccountKey;
    const allowedAccountKey = await getCommandManagementAccountKey(chatId, from.id, targetAccountKey);
    if (!allowedAccountKey) return commandManagementDenied(chatId);
    const commandKey = payload.command_key;
    const saved = await saveCommandTemplateEdit(allowedAccountKey, commandKey, text, from.id);
    await setSelectedAccountKey(chatId, allowedAccountKey, from.id);
    if (!saved) return tg('sendMessage', { chat_id: chatId, text: 'Buyruq topilmadi.' });
    await tg('sendMessage', { chat_id: chatId, text: 'Shablon muvaffaqiyatli yangilandi' });
    return sendCommandDetail(chatId, allowedAccountKey, saved.command_key || commandKey);
  }

  if (session?.mode === 'command_create' && !text.startsWith('/')) {
    await setSelectedAccountKey(chatId, selectedAccountKey, from.id);
    return tg('sendMessage', { chat_id: chatId, text: 'Yangi buyruq qo‘shish hozircha o‘chirilgan.' });
  }
  ...
  if ([
    'custom_command_key_input',
    'custom_command_response_text',
    'custom_command_test_input',
    'custom_command_trigger_select',
    'custom_command_response_select',
    'custom_command_template_select',
    'custom_command_preview'
  ].includes(session?.mode) && !text.startsWith('/')) {
    await setSelectedAccountKey(chatId, selectedAccountKey, from.id);
    return tg('sendMessage', { chat_id: chatId, text: 'Yangi buyruq qo‘shish va test qilish hozircha o‘chirilgan.' });
  }
  ...
  if (session?.mode === 'ai_rule_test_input' && !text.startsWith('/')) {
    ...
    if (!accountSetHas(ownedAccountKeys, fakeLead.account_key)) return denyAccount();
    await setSelectedAccountKey(chatId, fakeLead.account_key, from.id);
  ...
  if (text === '/accounts') return sendAccountsMenu(chatId, from.id);
  ...
  if (text.startsWith('/ai ')) {
    ...
    if (maybeAccount && !accountSetHas(ownedAccountKeys, targetAccount)) return denyAccount();
    if (mode === 'on') {
  ...
  if (text.startsWith('/aitemplate ')) {
    ...
    if (maybeAccount && !accountSetHas(ownedAccountKeys, templateAccountKey)) return denyAccount();
  ...
  if (text === '/commands') {
    const commandAccountKey = await getCommandManagementAccountKey(chatId, from.id, selectedAccountKey);
    if (!commandAccountKey) return commandManagementDenied(chatId);
    return sendCommandsMenu(chatId, commandAccountKey);
  }
  if (text === '/commands_debug') {
    const commandAccountKey = await getCommandManagementAccountKey(chatId, from.id, selectedAccountKey);
    if (!commandAccountKey) return commandManagementDenied(chatId);
    return tg('sendMessage', { chat_id: chatId, text: await customCommandsDebugText(commandAccountKey, session) });
  }
  if (text.startsWith('/testcommand ')) {
    const commandAccountKey = await getCommandManagementAccountKey(chatId, from.id, selectedAccountKey);
    if (!commandAccountKey) return commandManagementDenied(chatId);
    const sample = text.replace('/testcommand ', '');
    return tg('sendMessage', { chat_id: chatId, text: await customCommandDryRunText(commandAccountKey, sample) });
  }
  ...
  if (text.startsWith('/archive_debug ')) {
    ...
    const accounts = await getAccounts();
    const maybeAccount = accounts.find(a => a.account_key === parts[1]);
    if (maybeAccount && !accountSetHas(ownedAccountKeys, maybeAccount.account_key)) return denyAccount();
...
async function handleCallback(cb) {
  ...
  const denyAccount = () => commandManagementDenied(chatId);
  const commandCallback = isCommandManagementCallbackData(data);
  ...
  if (!ownedAccountKeys.size) {
    if (data === 'menu' || data === 'noop' || data === 'accounts') return sendAccountsMenu(chatId, cb.from?.id);
    if (commandCallback) return commandManagementDenied(chatId);
    return denyAccount();
  }
  ...
  if (data.startsWith('account_bot:')) {
    ...
    return sendDashboard(chatId, allowedAccountKey, cb.from?.id);
  }
  ...
  if (data === 'reach_start_confirm') return executeReachStart(chatId, cb.from?.id);
  if (data === 'reach_start_cancel') {
    await setSelectedAccountKey(chatId, selectedAccountKey, cb.from?.id);
  ...
  if (commandCallback) {
    ...
    if (data.startsWith('command_view:') || data.startsWith('cmd_open:') || data.startsWith('cmd_view:')) {
      return sendCommandDetail(chatId, commandAccountKey, data.split(':')[1]);
    }
    if (data.startsWith('cmd_edit_template:') || data.startsWith('cmd_edit_response:') || data.startsWith('command_edit_response:') || data.startsWith('cmd_editresp:')) {
      const commandKey = data.split(':')[1];
      const command = await getCustomCommand(commandAccountKey, commandKey, true);
      if (!command) return tg('sendMessage', { chat_id: chatId, text: 'Buyruq topilmadi.' });
      await setAdminSession(chatId, 'editing_command_template', {
        selected_account_key: commandAccountKey,
        account_key: commandAccountKey,
        command_key: command.command_key,
        return_to: 'command_details'
      });
      return tg('sendMessage', { chat_id: chatId, text: 'Yangi javob shablonini yuboring.' });
    }
    if (data.startsWith('cmd_archive:') || data.startsWith('command_archive:') || data.startsWith('command_delete:') || data.startsWith('cmd_delete:')) {
      const key = data.split(':')[1];
      const saved = await setCommandArchiveState(commandAccountKey, key, true, cb.from?.id);
      if (!saved) return tg('sendMessage', { chat_id: chatId, text: 'Buyruq topilmadi.' });
      await tg('sendMessage', { chat_id: chatId, text: `📦 /${saved.command_key || key} arxivlandi.` });
      return sendCommandDetail(chatId, commandAccountKey, saved.command_key || key);
    }
    if (data.startsWith('cmd_restore:') || data.startsWith('command_restore:')) {
      const key = data.split(':')[1];
      const saved = await setCommandArchiveState(commandAccountKey, key, false, cb.from?.id);
      if (!saved) return tg('sendMessage', { chat_id: chatId, text: 'Buyruq topilmadi.' });
  ...
  if (data.startsWith('lead_')) {
    ...
    if (!accountSetHas(ownedAccountKeys, selectedAccountKey)) return denyAccount();
  ...
  if (data.startsWith('account:')) {
    ...
    if (!allowedKey) return commandManagementDenied(chatId);
    const account = await getAccount(allowedKey);
    await setSelectedAccountKey(chatId, account.account_key, cb.from?.id);
    return tg('sendMessage', { chat_id: chatId, text: `✅ Akkaunt tanlandi: ${account.label || account.account_key}` });
  }
  ...
  if (data === 'flow_list') return sendFlow(chatId, selectedAccountKey);
  if (data === 'flow_add') {
    await setAdminSession(chatId, 'flow_display_input', { selected_account_key: selectedAccountKey });
    return tg('sendMessage', { chat_id: chatId, text: 'Bosqich nomini yuboring. Masalan: Ariza tasdig‘i' });
  }
  if (data === 'flow_reorder') return tg('sendMessage', { chat_id: chatId, text: 'Tartibni o‘zgartirish uchun hozircha /setflow yoki bosqichni qayta saqlashdan foydalaning.' });
  if (data === 'flow_test') {
    const steps = await getFlowSteps(selectedAccountKey);
    const rows = (steps.length ? steps : [{ step_key: STAGE.ASKED_APPLICATION }]).map(s => ([{ text: s.step_key, callback_data: `flow_test_step:${s.step_key}` }]));
    rows.push([{ text: '❌ Bekor qilish', callback_data: 'flow_cancel' }]);
    return tg('sendMessage', { chat_id: chatId, text: 'Qaysi bosqichdan test qilamiz?', reply_markup: { inline_keyboard: rows } });
  }
  if (data.startsWith('flow_test_step:')) {
    await setAdminSession(chatId, 'flow_test_input', { selected_account_key: selectedAccountKey, step_key: data.split(':')[1] });
    return tg('sendMessage', { chat_id: chatId, text: 'Sample mijoz javobini yuboring.' });
  }
  if (data.startsWith('flow_template:')) {
    const session = await getAdminSession(chatId);
    const payload = { ...(session?.payload || {}), template_key: data.split(':')[1] };
    await setAdminSession(chatId, 'flow_wait_select', payload);
    return tg('sendMessage', {
      chat_id: chatId,
      text: 'Bu xabardan keyin mijoz javobini kutamizmi?',
      reply_markup: {
        inline_keyboard: [
          [{ text: 'Ha', callback_data: 'flow_wait:yes' }, { text: 'Yo‘q', callback_data: 'flow_wait:no' }],
          [{ text: '❌ Bekor qilish', callback_data: 'flow_cancel' }]
        ]
      }
    });
  }
  if (data.startsWith('flow_wait:')) {
    const session = await getAdminSession(chatId);
    const payload = { ...(session?.payload || {}), wait_for_reply: data.endsWith(':yes'), stop_after_send: data.endsWith(':no') };
    return flowStepPreview(chatId, payload);
  }
  if (data === 'flow_save') {
    const session = await getAdminSession(chatId);
    const payload = session?.payload || {};
    const targetAccountKey = payload.selected_account_key || selectedAccountKey;
    if (!accountSetHas(ownedAccountKeys, targetAccountKey)) return denyAccount();
    if (!payload.step_key || !payload.template_key) return tg('sendMessage', { chat_id: chatId, text: 'Saqlanadigan flow preview topilmadi.' });
    await upsertFlowStep({
      accountKey: targetAccountKey,
      stepKey: payload.step_key,
      displayName: payload.display_name,
      templateKey: payload.template_key,
      waitForReply: payload.wait_for_reply,
      stopAfterSend: payload.stop_after_send,
      nextUnknown: payload.wait_for_reply ? 'human_needed' : null
    });
    await clearSetupSession(chatId, targetAccountKey);
    return tg('sendMessage', { chat_id: chatId, text: `✅ Flow bosqichi saqlandi: ${payload.step_key}` });
  }
  if (data === 'flow_retry') {
    await setAdminSession(chatId, 'flow_display_input', { selected_account_key: selectedAccountKey });
    return tg('sendMessage', { chat_id: chatId, text: 'Bosqich nomini qayta yuboring.' });
  }
  if (data === 'flow_cancel') {
    await clearSetupSession(chatId, selectedAccountKey);
    return tg('sendMessage', { chat_id: chatId, text: 'Bekor qilindi.' });
  }
  if (data === 'ai_menu') {
  ...
  if (data === 'airule_add' || data === 'airule_ai_create') {
    await setAdminSession(chatId, 'ai_rule_natural_input', { selected_account_key: selectedAccountKey });
    return tg('sendMessage', { chat_id: chatId, text: `${!OPENAI_API_KEY && data === 'airule_ai_create' ? 'AI sozlanmagan. Bot oddiy taklif yaratadi.\n\n' : ''}Qoidani oddiy tilda yozing. Masalan: Agar mijoz ha yoki qiziqdim desa, full_intro shablonini yuborib keyin to‘xtasin.` });
  }
  if (data === 'airule_save') {
    const session = await getAdminSession(chatId);
    const payload = session?.payload || {};
    const targetAccountKey = payload.selected_account_key || selectedAccountKey;
    if (!accountSetHas(ownedAccountKeys, targetAccountKey)) return denyAccount();
    const saved = await upsertAccountAiRule(targetAccountKey, payload.rule || {}, cb.from?.id);
    await clearSetupSession(chatId, targetAccountKey);
    return tg('sendMessage', { chat_id: chatId, text: `✅ AI qoida saqlandi: ${saved.rule_key}` });
  }
  if (data === 'airule_retry') {
    await setAdminSession(chatId, 'ai_rule_natural_input', { selected_account_key: selectedAccountKey });
    return tg('sendMessage', { chat_id: chatId, text: 'Qoidani qayta yozing.' });
  }
  if (data === 'airule_cancel') {
    await clearSetupSession(chatId, selectedAccountKey);
    return tg('sendMessage', { chat_id: chatId, text: 'Bekor qilindi.' });
  }
  ...
  if (data === 'tpl_add') return startTemplateAdd(chatId, selectedAccountKey);
  if (data === 'tpl_test_menu') return tg('sendMessage', { chat_id: chatId, text: 'Test uchun shablonni ro‘yxatdan tanlab, “🧪 Test yuborish” tugmasini bosing.' });
  ...
  if (data.startsWith('tpl_softdelete:')) {
    const key = data.split(':')[1];
    await setTemplate(key, '[deleted]', selectedAccountKey);
    return tg('sendMessage', { chat_id: chatId, text: `🗑 ${selectedAccountKey}/${key} o‘chirilgan holatga o‘tkazildi.` });
  }
  if (data === 'tpl_ai_save') {
    const session = await getAdminSession(chatId);
    const payload = session?.payload || {};
